...
search_space_wine = [
    ...
    (
        X_train["residual_sugar"].min(),
        X_train["residual_sugar"].max(),
    ),  # residual_sugar
    (X_train["chlorides"].min(), X_train["chlorides"].max()),  # chlorides
    (
        X_train["free_sulfur_dioxide"].min(),
        X_train["free_sulfur_dioxide"].max(),
    ),  # free_sulfur_dioxide
    (
        X_train["total_sulfur_dioxide"].min(),
        X_train["total_sulfur_dioxide"].max(),
    ),  # total_sulfur_dioxide
...
def run_hitl_optimization(
    ...
    for t in range(1, n_iterations + 1):
        ...
        candidates = [
            [np.random.uniform(low, high) for low, high in search_space_wine_xgb]
        ...
        def acquisition_function(x):
            ...
            return (1 - w_human) * pred_quality + w_human * pred_preference
        ...
        if len(evaluated_points) > 1:
            ...
            # The user belief model is trained on pairs of (preferred_profile - non_preferred_profile)
            # with a label of 1.
    ...
    return best_params_hitl, best_quality_hitl, convergence_hitl
...
print(f"  Best Quality: {best_quality_helpful:.4f}")

print(f"\nHITL with Adversarial Expert:")
print(f"  Best Quality: {best_quality_adversarial:.4f}")
...
print(
    ...
    "Interestingly, the adversarial feedback forced the model to explore regions it would have otherwise ignored, leading it to discover a solution even better than the one found with the helpful expert. "
    "This demonstrates that not only is the system robust against incorrect input, but that forced exploration can sometimes uncover superior results by preventing premature convergence on a local optimum."
)
